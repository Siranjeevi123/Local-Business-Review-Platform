...
function saveReviews($reviews) {
    global $reviewsFile;
    return file_put_contents($reviewsFile, json_encode($reviews, JSON_PRETTY_PRINT));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Simple validation
    if (strlen($data['reviewer_name']) < 2) {
        echo json_encode(['success' => false, 'message' => 'Name must be at least 2 characters']);
        exit;
    }
    
    if (!filter_var($data['reviewer_email'], FILTER_VALIDATE_EMAIL)) {
        echo json_encode(['success' => false, 'message' => 'Invalid email format']);
        exit;
    }
    
    if (strlen($data['review-title']) < 2) {
        echo json_encode(['success' => false, 'message' => 'Title must be at least 2 characters']);
        exit;
    }
    
    if (strlen($data['review-content']) < 2) {
        echo json_encode(['success' => false, 'message' => 'Review content must be at least 2 characters']);
        exit;
    }
    ...
    // Create new review - set approved to true immediately
    $newReview = [
        'id' => (string)$newId,
    ...
    // Save reviews
    $saveResult = saveReviews($reviews);
    
    echo json_encode(['success' => true, 'message' => 'Review submitted successfully']);
    exit;
}

// Handle PUT requests (update a review)
...
// Handle DELETE requests (delete a review)
