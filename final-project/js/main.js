...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (path.endsWith("index.html") || path.endsWith("/") || path === "") {
    ...
    // These are already called above
    ...
    if (businessId) {
      ...
      document.getElementById("business-detail-container").innerHTML =
        '<div class="error">No business ID provided. Please go back to the <a href="business-listing.html">business listing</a>.</div>'
    }
  }
})
...
function loadRelatedBusinesses(businessId) {
  ...
  container.innerHTML = '<div class="loading">Loading related businesses...</div>'
  ...
  // Filter out the current business and get 3 random businesses
  ...
  relatedBusinesses.forEach((business) => {
    html += `
      <div class="business-card stagger-item">
        <div class="business-image" style="background-image: url('${business.image}')">
          <div class="business-category-tag">${business.category}</div>
        </div>
        <div class="business-info">
          <h3 class="business-name">${business.name}</h3>
...
function setupReviewForm(businessId) {
  ...
  reviewForm.addEventListener("submit", (e) => {
    ...
    setTimeout(() => {
      ...
      // Add the new review to the page immediately
      const reviewsContainer = document.getElementById("reviews-container")
      const noReviews = reviewsContainer.querySelector(".no-reviews")

      // If there were no reviews before, replace the "no reviews" message
      if (noReviews) {
        reviewsContainer.innerHTML = ""
      }
      ...
      const reviewHTML = `
        <div class="review-card">
          <div class="review-header">
            <div class="reviewer-info">
              <div class="reviewer-avatar">${getInitials(newReview.reviewer_name)}</div>
              <div>
                <div class="reviewer-name">${newReview.reviewer_name}</div>
      ...
      const reviewCountElement = document.getElementById("review-count")
      if (reviewCountElement) {
        const currentCount = Number.parseInt(reviewCountElement.textContent.split(" ")[0]) || 0
        const newCount = currentCount + 1
        reviewCountElement.textContent = `${newCount} ${newCount === 1 ? "review" : "reviews"}`
      }
      ...
      reviewFormContainer.innerHTML = `
        <div class="submission-success">
          ...
          <p>Your review has been submitted and is now visible on the page.</p>
